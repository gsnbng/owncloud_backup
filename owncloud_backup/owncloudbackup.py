...
class BackupGenerator:
	...
	def get_backup(self, older_than=24, ignore_files=False, force=False):
		...
		if not (last_db and last_file and last_private_file):
			"""
			No database backup for owncloud
			self.take_dump()
			"""
			if not ignore_files:
				self.zip_files()
	...
	def set_backup_file_name(self):
		...
		for_private_files = todays_date + "_owncloud_" + random_string + "_private_files.tar"
	...
	def zip_files(self):
		for folder in ("private"):
			...
			cmd_string = """tar -cf %s %s""" % (backup_path, frappe.db.get_value("Owncloud path",None,"owncloud_path"))
...
def get_backup_path():
	backup_path = frappe.utils.get_site_path(conf.get("backup_path", "private/backups/owncloud"))
	return backup_path
